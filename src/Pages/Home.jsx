...
function Home() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await fetch("https://ai-images-change-detetction-backend-83m0bw.fly.dev/api/compare", {
        method: "POST",
        body: formData,
      });
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const response = await axios.post(
        "https://ai-images-change-detetction-backend-83m0bw.fly.dev/api/report", // تأكد من الـ URL صحيح
        {
          image1Base64,
          image2Base64,
          diffImageBase64,
