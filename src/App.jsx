...
//               <div dir="rtl" className="bg-gradient-to-br from-white/10 to-white/5 p-8 rounded-2xl border border-white/10 backdrop-blur-sm">
...
function App() {
  const [image1, setImage1] = useState(null);
  const [image2, setImage2] = useState(null);
  const [diffImage, setDiffImage] = useState(null); // For displaying the image with data: prefix
  const [rawDiffImageBase64, setRawDiffImageBase64] = useState(null); // For sending raw base64 to backend
  const [report, setReport] = useState("");
  const [loading, setLoading] = useState(false); // For image comparison loading
  const [exportLoading, setExportLoading] = useState(false); // For shapefile export loading
  const [error, setError] = useState(null);

  // Dimensions of the processed images (hardcoded as per server.js)
  ...
  const handleUpload = async (e) => {
    ...
    setLoading(true);
    setDiffImage(null);
    setRawDiffImageBase64(null); // Reset raw base64 on new upload
    setReport("");
    setError(null);
    ...
    try {
      ...
      setRawDiffImageBase64(data.diffImage.replace(imagePrefix, '')); 
      setReport(data.summary);
    } catch (err) {
      setError("Error: " + err.message);
    } finally {
  ...
  const handleExportShapefile = async () => {
    if (!rawDiffImageBase64) {
      setError("No difference image available to export.");
      return;
    }
    ...
    try {
      const response = await fetch("http://localhost:3000/api/export-shapefile", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          diffImageBase64: rawDiffImageBase64,
          width: IMAGE_WIDTH,
          height: IMAGE_HEIGHT,
        }),
      });
      ...
      // The response is a ZIP file, so get it as a Blob
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "change-detection-shapefile.zip"; // Suggested filename
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url); // Clean up the object URL

      console.log("Shapefile exported successfully!");
    } catch (err) {
      console.error("Error exporting Shapefile:", err);
      setError("Error exporting Shapefile: " + err.message);
    } finally {
  ...
  const handleImageReset = () => {
    ...
    setImage2(null);
    setDiffImage(null);
    setRawDiffImageBase64(null); // Reset raw base64
    setReport("");
    setError(null);
  };
            ...
            <div className="group">
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-8 transition-all duration-500 hover:shadow-orange-500/25 hover:shadow-2xl hover:scale-[1.02] hover:bg-white/15">
                <div className="flex items-center justify-between mb-6"> {/* Added justify-between */}
                  ...
                  {/* New Export Shapefile Button */}
                  <button
                    onClick={handleExportShapefile}
                    disabled={exportLoading}
                    className={`group relative overflow-hidden py-3 px-6 rounded-xl text-white font-semibold text-base transition-all duration-300 shadow-md transform ${
                      exportLoading
                        ? "bg-red-600 cursor-not-allowed shadow-none"
                        : "bg-gradient-to-r from-green-500 to-teal-500 hover:from-green-600 hover:to-teal-600 hover:shadow-lg hover:shadow-teal-500/25 active:scale-95 hover:scale-105"
                    } flex items-center justify-center`}
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                    {exportLoading ? (
